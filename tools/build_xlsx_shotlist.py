...
MAP = {
 ...
 ("01",6):("屋外・街","通勤路・駅・会社前（①・同じ画角を使い回す）",CAM+"／レフ板"),
 ("01",7):("白台・物撮り","四色を並べる（①の締め）",CAM+"／照明1灯／白台"),
 ...
 ("05",2):("複写・資料","沿革の写真を複写する（⑤）",CAM+"／自然光"),
 ("05",3):("企画・設計","革を選ぶ・色を決める（⑤）",CAM+"／照明1灯"),
 ("05",4):("企画・設計","机まわり・図面と数値（⑤⑥共通）",CAM+"／照明1灯"),
 ("05",5):("工房・全工程","一工程一カットで流す（⑤は簡潔に）",CAM),
 ("05",6):("店舗","BROOKLYN MUSEUM 店舗（⑤⑧共通）",CAM+"／ガンマイク"),
 ("05",7):("お客様・街","受け取りと使用（⑤⑧共通）",CAM),
 ("05",8):("お客様・街","締め（⑤）",CAM),
...
# シーン単位ではなくコマ単位で場所が変わるもの（インタビューのカットなど）
SHOT_MAP = {
 ("06",1,3):("インタビュー","佐藤さん（⑥・工房案内／②にも出演）",ITV),
 ("06",8,3):("インタビュー","佐藤さん（⑥・工房案内／②にも出演）",ITV),
 ("05",1,3):("インタビュー","永尾社長（⑤・冒頭と締めの二カット）",ITV),
 ("05",5,1):("インタビュー","佐藤さん（②に出演・声と手元）",ITV),
 ("05",6,1):("インタビュー","くさがやさん（⑤⑥）",ITV),
 ("05",8,1):("インタビュー","永尾社長（⑤・冒頭と締めの二カット）",ITV),
}
...
EXTRA = [
 ("インタビュー","三人共通の撮り方",ITV,"②⑤⑥",
  "画角を三つ撮る：①バストショット（人物を左右に寄せ、空いた側にテロップ）②引き（背景に社名ロゴの壁）③横顔"),
 ("インタビュー","三人共通の撮り方",ITV,"②⑤⑥",
  "背景は必ず大きくボカす。明るい室内・窓・緑を背にする。役職テロップは日英二行"),
 ("インタビュー","佐藤さん（②に出演・声と手元）",ITV,"②",
  "この仕事を20年続けている理由を聞く。②S6で一言だけ使う（台本なし）"),
 ("インタビュー","佐藤さん（②に出演・声と手元）",ITV,"②",
  "作業音は別録りが必要なので、インタビューと手元カットは別のセッションにする"),
 ("インタビュー","佐藤さん（⑥・工房案内／②にも出演）",ITV,"⑥",
  "工房を歩きながら案内してもらう。台本は作らない。⑥の全編に流し続けるナレーション素材になる"),
 ("インタビュー","佐藤さん（⑥・工房案内／②にも出演）",ITV,"⑥",
  "強みを三つ話してもらう：小ロットから量産まで／自社で完結／誰が作っているか顔が見える"),
 ("インタビュー","くさがやさん（⑤⑥）",ITV,"⑤⑥",
  "漉き・仕上げの仕事について一言。顔出しが難しければ手元だけのバージョンも撮る"),
 ("インタビュー","永尾社長（⑤・冒頭と締めの二カット）",ITV,"⑤",
  "30分収録して、経営方針の本音を拾う。台本の読み上げにしない"),
 ("インタビュー","永尾社長（⑤・冒頭と締めの二カット）",ITV,"⑤",
  "創業から今までの変遷。⑤の全編に流し続けるナレーション素材になる"),
 ("インタビュー","永尾社長（⑤・冒頭と締めの二カット）",ITV,"⑤",
  "締めの一言を、収録の最後に一本言い切ってもらう（三十分の収録から抜くのではなく、締め用に撮る）"),
]
    ...
    for page in data["pages"]:
        ...
        for si, r in enumerate(page["rows"], start=1):
            ...
            for hi, s in enumerate(shots, start=1):
                place, setname, gear = SHOT_MAP.get((no, si, hi)) or MAP[(no, si)]
