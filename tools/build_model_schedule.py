...
PLACE = "BROOKLYN MUSEUM 表参道店（店休日）・表参道けやき並木　→　代官山"
...
ROWS = [
 ...
 ("9:00〜9:30", "BROOKLYN MUSEUM 表参道店（店内）", MODEL, "一色目の衣装・鞄",
  "支度", "着替えて、髪を整える。カメラを置く位置と、歩いてもらう道を先に見てもらう。", "三十分",
  "<b>この日は店休日なので、一日、店内を使えます。</b>着替えも荷物置き場も店内です。"),

 ("9:30〜10:05", "店内のレジまわり　→　店の前", MODEL + "、店の人", "商品、鞄",
  "⑤ S6 購入・使う", cam("05", 6), sec("05", 6),
  "<b>店の人と二人で映るのは、ここだけです。</b>①の一色目のまま撮ります。セリフはありません。"),

 ("band", "① 　四 色 を 一 巡 ず つ　（ 外　→　店 の 扉　→　店 内 ）"),

 ("10:05〜10:40", "けやき並木の歩道　→　店の扉　→　店内", MODEL, "一色目の衣装・鞄",
  "① S1 一色目",
  "1枚目 歩いてくる全身の引き・正面（並木が奥まで抜ける位置）→ 2枚目 同じ位置から腰より上 → "
  "3枚目 扉をまたいで中へ入る → 4枚目 店内を歩く → 5枚目 定位置の立ち姿。", sec("01", 1),
  "<b>ここから四回、色を変えます。</b>表情はすまし顔で。"),

 ("10:40〜11:15", "横断歩道（交差点から離れた端）　→　店の扉　→　店内", MODEL, "二色目の衣装・鞄",
  "① S2 二色目",
  "1枚目 道を渡りきるところの引き → 2枚目 腰より上（サングラスを直す）。"
  "扉・店内・定位置は一色目と同じ。", sec("01", 2),
  "<b>ここは笑った表情で。</b>着替え十分。"),

 ("11:15〜11:50", "並木の木ぎわ　→　店の扉　→　店内", MODEL, "三色目の衣装・鞄",
  "① S3 三色目",
  "1枚目 木の後ろを通って歩く引き → 2枚目 腰より上。扉・店内・定位置は同じ。", sec("01", 3),
  "<b>少し急ぎ足の表情で。</b>着替え十分。"),

 ("11:50〜12:25", "店の前　→　店の扉　→　店内", MODEL, "四色目の衣装・鞄",
  "① S4 四色目",
  "1枚目 扉をまたいで中へ入る引き → 2枚目 腰より上（鞄を掛け直して笑う）。店内と定位置も同じ。",
  sec("01", 4), "<b>四色とも同じ立ち位置・同じ画角で撮ります。</b>着替え十分。"),

 ("12:25〜12:35", "店内（定位置）", MODEL, "四色ぶんの衣装・鞄",
  "① S5 巻き戻る", "店内の通路を歩くところを、色ごとに同じ画角で撮り足す。", sec("01", 5),
  "<b>四色を並べる最後のカットは、難波さんが帰ったあとに撮ります。</b>"),

 ("12:35〜13:20", "昼休み（表参道）", "―", "",
  "―", "", "45分", "<b>昼休みのあいだに、④の一そろいに着替えます。</b>"),

 ("13:20〜13:45", "移動（表参道　→　代官山）", "―", "",
  "―", "", "25分", "荷物は持って出ます。<b>撮影は代官山で終わりです。</b>"),

 ("band", "午 後　──　代 官 山"),

 ("13:45〜14:05", "代官山（柵か手すりのあるところ）", MODEL, "鞄、柵",
  "④ S1 街に置く",
  "1枚目 柵に掛ける（引き）→ 2枚目 肩に掛けて立つ（横・引き）→ 3枚目 街路樹の下・全身。",
  sec("04", 1),
  "<b>最初と最後は同じ柵。</b>カメラは止めたままで、動くのは背景の街だけです。"),

 ("14:05〜14:25", "S1と同じ柵", MODEL, "鞄、持ち物、靴、ロゴ",
  "④ S4 立ち止まって終わる", cam("04", 4), sec("04", 4),
  "靴と持ち物も一緒に並べて撮ります。"),

 ("14:25〜14:50", "ベンチか石段（真上から撮れるところ）", MODEL, "鞄、中に入れる持ち物",
  "④ S2 中を見せる", cam("04", 2), sec("04", 2),
  "<b>手が真俯瞰で映ります。</b>鞄に入れる持ち物は、先に並べておきます。"),

 ("14:50〜15:10", "代官山の裏通り（人がほとんど通らない側）", MODEL, "鞄",
  "④ S3 街を歩く", "1枚目 通りを歩く（引き・固定）→ 2枚目 肩の鞄に寄る。", sec("04", 3),
  "人の少ない裏側で。顔は主役にしません。"),
]
