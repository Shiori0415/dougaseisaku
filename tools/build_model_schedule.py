...
PLACE = "BROOKLYN MUSEUM 表参道店（店休日）・南青山　→　代官山（車で移動）"
...
ROWS = [
 ("band", "午 前　──　店 舗 と 、 南 青 山"),

 ("9:00〜9:30", "BROOKLYN MUSEUM 表参道店（店内）", MODEL, "④の衣装 一そろい",
  "支度", "着替えて、髪を整える。この日の流れと、歩いてもらう道を先に見てもらう。", "三十分",
  "<b>この日は店休日なので、午前中は店内を使えます。</b>"),

 ("9:30〜10:10", "店内　→　店の前", MODEL + "、店の人", "商品、鞄",
  "写真撮影（スチール）", "店内の商品と、鞄を持つ姿を静止画で撮る。", "40分",
  "<b>動画より先に写真を済ませます。</b>カメラに慣れる時間にもなります。"),

 ("10:10〜10:40", "店内のレジまわり　→　店の前", MODEL + "、店の人", "商品、鞄",
  "⑤ S6 購入・使う", cam("05", 6), sec("05", 6),
  "<b>店の人と二人で映るのは、ここだけです。</b>買う人の役。セリフはありません。"),

 ("10:40〜11:05", "店内の台（真上から撮れるところ）", MODEL, "鞄、中に入れる持ち物",
  "④ S2 中を見せる", cam("04", 2), sec("04", 2),
  "<b>手が真俯瞰で映ります。</b>鞄に入れる持ち物は、先に並べておきます。"),

 ("11:05〜11:25", "店の前　＋　店内（置き画）", MODEL, "鞄、持ち物、靴、ロゴ",
  "④ S4 立ち止まって終わる",
  "1枚目 立ち止まる（店の前・固定）→ 2枚目 鞄と持ち物の置き画（店内・真俯瞰）→ 3枚目 ロゴ。",
  sec("04", 4), "靴と持ち物も一緒に並べて撮ります。"),

 ("11:25〜11:45", "店の前の柵・手すり（けやき並木側）", MODEL, "鞄、柵",
  "④ S1 街に置く",
  "1枚目 柵に掛ける（引き）→ 2枚目 肩に掛けて立つ（横・引き）→ 3枚目 並木の下・全身。",
  sec("04", 1), "<b>最初と最後は同じ柵。</b>カメラは止めたままで、動くのは背景の街だけです。"),

 ("11:45〜12:15", "みゆき通り　／　根津美術館前の坂（南青山）", MODEL, "鞄",
  "④ S3 街を歩く", "1枚目 通りを歩く（引き）→ 2枚目 肩の鞄に寄る。", sec("04", 3),
  "<b>人の少ない通りで。</b>ここは手持ちで撮ります。顔は主役にしません。"),

 ("12:15〜13:00", "昼休み（店舗の近く）", "―", "",
  "―", "", "45分", "<b>荷物と衣装は、昼のあいだに車へ積みます。</b>"),

 ("13:00〜13:30", "移動（表参道　→　代官山）", "―", "",
  "―", "", "30分", "車で十五分ほど。<b>近くのコインパーキングに停めます。</b>"),

 ("band", "午 後　──　代 官 山 　（ 四 色 を 一 色 ず つ 。 着 替 え は 車 の 中 ）"),

 ("13:30〜14:00", "旧山手通りのけやき並木（歩道）", MODEL, "一色目の衣装・鞄",
  "① S1 一色目",
  "1枚目 全身の引き・正面（並木が奥まで抜ける位置）→ 2枚目 同じ位置から腰より上。"
  "街路樹の幹の裏を通った一コマでつなぐ。", sec("01", 1),
  "<b>ここから四回、色を変えます。</b>表情はすまし顔で。着替えは車の中、十分。"),

 ("14:00〜14:30", "ヒルサイドテラス前の歩道（白い壁と列柱を背に）", MODEL, "二色目の衣装・鞄",
  "① S2 二色目",
  "1枚目 引き → 2枚目 膝から上。柱の裏を通った一コマでつなぐ。", sec("01", 2),
  "<b>敷地には入りません。歩道から撮ります。</b>ここは笑った表情で。"),

 ("14:30〜15:00", "猿楽町の坂と階段", MODEL, "三色目の衣装・鞄",
  "① S3 三色目",
  "1枚目 坂を上ってくる引き → 2枚目 腰より上。手すりの前を通った一コマでつなぐ。",
  sec("01", 3), "<b>少し急ぎ足の表情で。</b>色が変わるごとに、一段ずつ寄っていきます。"),

 ("15:00〜15:30", "八幡通りの軒下（暗い区間）", MODEL, "四色目の衣装・鞄",
  "① S4 四色目",
  "1枚目 軒下に入っていく引き → 2枚目 胸から上（鞄を掛け直して笑う）。", sec("01", 4),
  "<b>四色のなかで、ここがいちばん寄ります。</b>"),

 ("15:30〜15:45", "代官山駅前の歩道（人通りのあるところ）", MODEL, "四色ぶんの衣装・鞄",
  "① S5 巻き戻る",
  "1枚目 歩道を歩く（人が横切るたびに一つ前の服に戻る）→ 2枚目 腰より上。", sec("01", 5),
  "<b>ここまでで難波さんは終わりです。</b>四色を並べる最後のカットは、"
  "難波さんが帰ったあとに店で撮ります。"),
]
...
def info_html():
    ...
    tel = ('<span style="font-size:11pt;font-weight:700;color:%s">%s</span>' % (GOLD, TEL))
    pairs = [("タイトル", TITLE, "", ""),
