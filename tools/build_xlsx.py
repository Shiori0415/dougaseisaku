...
GEAR = [
    ...
    ("三脚（雲台付き）", "Velbon EX-440", 3012,
     ...
     "国内カメラ用品の老舗メーカー・入門定番。価格は約3,000円だが、6,018円の掲載もあり要確認",
    ...
    ("音声レコーダー", "ZOOM H1essential", 12626,
     ...
     "ZOOM公式。評価4.53／5・32件（Yahoo!ショッピング掲載値。Amazonの評価ではありません）。32bitフロートで音割れしない",
    ...
    ("レフ板", "EMART レフ板 60cm 丸レフ板 5in1", 1099,
     ...
     "レビュー件数の多い定番品。今回の再確認では実売価格を確定できず、1,099円は前回調査時の値",
...
def build_gear(path):
    ...
    for line in [
        ...
        "・価格の確度：マクロレンズ・レコーダー・ガンマイク・照明・黒スチレンボードは今回の再確認で実売価格を確認済み。",
        "　三脚とレフ板は確定できず（三脚は約3,000円と6,018円の両方の掲載あり）、アクリル板とSDカードは概算です。",
        "",
        "商品ページのリンクについて",
