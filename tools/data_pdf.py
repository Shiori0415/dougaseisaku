...
PAGES = [
 ...
 dict(no="05", jp="1979年から、東京で。", en="Since 1979, Tokyo",
  ...
  rows=[
   ("S1 刻印・沿革","0-12秒",[
     ("5_ref_15_未来の刻印_金属マクロ","黒。刻印が押され、革に沈む（超マクロ）<br><span style='color:#a8763e'>ナレーション開始</span>"),
     ("5_ref_01_年号テロップ_LNG船1985","窪みに光が差す。そこに<b>年号テロップ</b>が重なる<br><span style='color:#8a7a63'>※年号の出し方はこの画角</span>"),
     ("5_ref_08_工場内_大型装置ハイアングル","引くと向島工房の全景になる（ハイアングル）"),
   ]),
   ("S2 製造・全員の職人","12-40秒",[
     ("5_ref_12_図面の束_俯瞰気味","原反が広げられる（真俯瞰）"),
     ("5_ref_14_職人がボール盤で作業_引き","裁断→縫製→コバ磨き。<b>職人を一人ずつ</b>、複数カットで（鍛治島参考の画角）"),
   ...
   ("S4 使われる・締め","60-75秒",[
     (None,"購入され、店員から手渡される瞬間（撮影して差し替え）"),
     (None,"実際に使われている手元（撮影して差し替え）<br><span style='color:#a8763e'>ナレーション最後の一言</span>"),
     ("5_ref_16_締めのテロップ_造るを創る","ナレーションが終わり → テロップ → ロゴ"),
   ]),
  ], pn=7, total=T),
 ...
 dict(no="06", jp="つくる、確かめる、包む。", en="Make. Check. Pack.",
  ...
  rows=[
   ("S1 入る","0-25秒",[
     ("5_ref_03_工場全景_省力化テロップ","工房の外観と看板（ローアングル）。街の音"),
     ("5_S1_03_工房の全景ハイアングル","中に入る。工房の全景（ハイアングル）。<b>規模が分かる画角</b>"),
 ...
 dict(no="08", jp="購入者インタビュー（仮タイトル）", en="Customer Interview (Title TBD)",
  ...
  rows=[
   ("S1 導入","秒数未定",[
     ("8_S1_01_店を出る","日常の中で迎える（来店・使用シーンなど）"),
     ("8_S1_02_袋を持って歩く","名前は出さず「使っている人」として自己紹介"),
     ("8_S1_03_袋に寄る","手元のBROOKLYNの鞄が画面に入る"),
   ]),
   ("S2 質問","秒数未定",[
     ("8_S2_01_車内_膝の上に袋","「なぜこれを選んだか」を聞く"),
     ("8_S2_02_窓の外が流れる","実際に使っているシーンを見せながら答えてもらう"),
     (None,"良かった点を具体的なエピソードで語ってもらう"),
   ]),
   ("S3 本音","秒数未定",[
     ("8_S3_01_リボンをほどく","購入前に迷っていた話"),
     ("8_S3_02_開けた瞬間","買ってから変わったことを一言で"),
     ("8_S3_03_光にかざす","表情のアップ。台本にない一言を拾う"),
   ]),
   ("S4 締め","秒数未定",[
     (None,"鞄を持って歩き出す後ろ姿"),
     (None,"テロップ → ロゴ"),
