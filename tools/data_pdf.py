...
PAGES = [
 dict(no="01", jp="今日の服に、合う色。", en="A Colour for Every Outfit",
  ...
  rows=[
   ...
   ("S2 出る","3-6秒",[
     ...
     (None,"歩き出す足元（ロー）。手元の小物が一瞬だけ入る"),
     (None,"引きの全身。<b>この画角を最後まで使い回す（基準カット）</b>"),
   ]),
   ("S3 変わる①","6-10秒",[
 ...
 dict(no="04", jp="絵になる日常を。", en="Everyday, Worth a Frame",
  ...
  rows=[
   ...
   ("S2 つなぐ","6-11秒",[
     ...
     (None,"抜けたら、別の場所を歩いている"),
     (None,"看板・柱がフレームを横切る（二度目のつなぎ）"),
   ]),
   ("S3 中身","11-18秒",[
 ...
 dict(no="06", jp="つくる、確かめる、包む。", en="Make. Check. Pack.",
  ...
  rows=[
   ("S1 空から入る","0-10秒",[
     ...
     (None,"高度を下げ、外観と看板へ（ローアングルに切り替え）。街の音"),
   ...
   ("S2 工房の全体","10-25秒",[
     ...
     (None,"<b>Bロール：職人の顔。</b>手を止めずに作業している横顔（寄り）"),
   ]),
   ("S3 革を選ぶ・金型","25-42秒",[
   ...
   ("S7 検品 ★最重要","102-122秒",[
     ...
     (None,"<b>Bロール：職人の顔。</b>確かめ終えて顔を上げる一瞬。テロップ「検品 Inspection」"),
   ]),
   ("S8 梱包","122-133秒",[
 ...
 dict(no="08", jp="購入者インタビュー（仮タイトル）", en="Customer Interview (Title TBD)",
  ...
  rows=[
   ...
   ("S2 話しはじめる","秒数未定",[
     (None,"座って正面。名前は出さず「使っている人」として自己紹介"),
     (None,"手元の BROOKLYN MUSEUM の鞄が画面に入る"),
     (None,"「なぜこれを選んだか」を聞く"),
   ]),
   ("S3 使っている場面","秒数未定",[
