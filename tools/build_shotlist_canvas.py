...
DAYS = [
 dict(key="d2", no="10月1日", place="店舗・事務所", theme="開店前と閉店後に撮る。購入シーンだけ営業中",
      ...
      setups=[("9:00〜9:20", "【開店前】店内（ショーウィンドウ越し・棚）", [("05", 5)],
              ...
              ("10:30〜11:10", "【開店前】店舗の机（打ち合わせの場所）", [("05", 2), ("05", 3)],
               "机は一度組んで二シーン続けて。"),
              ("11:10〜20:00", "空き（編集にあてる）", [],
               "編集にあてる。"),
              ("20:00〜21:15", "【閉店後】事務所（バックヤード）の机・正面固定",
 ...
 dict(key="d4", no="10月2日", place="東京国際フォーラム → 表参道",
      ...
      cond="出演者と衣装・鞄 四色ぶんが揃ってから。会場の撮影申請も先に。曇りの日に合わせる。予備日は10月6日（火）の日中。",
      ...
      setups=[("9:00〜10:10", "ガラス棟ロビー（北端 → 柱の列 → ガラスの扉 → 南端の階段前）",
              ...
              ("14:35〜14:55", "けやき並木の裏の路地（人がほとんど通らない側）", [("04", 3)], "人の少ない裏側で。"),
 ...
 dict(key="d1", no="10月6日", place="向島工房、または店舗内の作業台", theme="工房が空いてから、一灯で撮る",
      date="10月6日（火）　※仮・終業後、または休日", date2="2026年10月6日（火）　※仮・終業後、または休日",
      toc=[("03", "五シーンすべて（工房の隅、または店舗内の作業台）")],
      cond="<b>工房は終業後か休日。工房が空かなければ店舗内の作業台（閉店後）でもよい。</b>",
      gear="カメラ／三脚／白い板／黒スチレンボード／ライト一灯",
      keys=["工房の人が帰ってから、または休日。店舗なら閉店後。",
            "ライト一灯だけ。当たっていない側は黒く落とす。",
            "三脚は必須。ピントは手で合わせて固定。",
            "画に入れるのはブルックリンの品だけ。"],
      setups=[("17:40〜19:20", "工房の隅、または店舗内の作業台",
...
# 実際に描かせて測った高さ
HEIGHT = {"cover": 873, "d2": 1680, "d5": 1490, "d4": 1960, "d3": 2644, "d1": 1253, "cast": 1217}
