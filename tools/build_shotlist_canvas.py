...
DAYS = [
 ...
 dict(key="d2", no="二日目", place="店舗", theme="購入者と、くさがやさんの手仕事",
      ...
      setups=[('9:00〜9:45', '店の前（外から中へ・見送り）', [("08", 1), ("08", 7)],
              ...
              ('9:55〜10:20', '店の前の通り', [("08", 5)],
               '使っている場面。外で動いてもらって、体をほぐす。'),
              ('10:30〜11:05', '店内のレジまわり', [("05", 6)],
               '会計から使う場面まで。<b>ここまでで、話す前にカメラに慣れてもらう。</b>'),
              ('11:15〜12:25', '店の窓辺（手前に植物を置いてボカす）',
               [("08", 3), ("08", 4), ("08", 6)],
               '<b>ここからインタビュー。</b>三シーン続けて。カメラは動かさない。購入者はここまで。'),
              ('12:25〜13:15', '昼休み', [], ''),
              ('13:15〜13:30', 'Bロール（まとめて撮る）', [("08", 2)],
               '人が映らないカット。③の台に組み替える前に済ませる。'),
              ('13:30〜14:25', '店舗の作業台（②の下仕事とコバ塗り）', [("02", 5), ("02", 6)],
               'くさがやさんの手。コバ塗りは切らずに六秒回す。'),
              ('14:25〜16:07', '店舗内の作業台（③・黒スチレンボード）',
               [("03", 1), ("03", 2), ("03", 3), ("03", 5)],
               '<b>くさがやさんの手。S4だけ四日目に工房で撮る。</b>'
               '人が入る画は肩から先だけ。')]),
 ...
 dict(key="d4", no="四日目", place="向島工房（休日）", theme="②を工程順に、静かな工房で",
      ...
      setups=[("9:30〜9:50", "革のロールが並ぶ棚", [("02", 1)], "一枚だけ、三秒。"),
              ...
              ("13:35〜13:55", "工房の一角（③・黒スチレンボード）", [("03", 4)],
