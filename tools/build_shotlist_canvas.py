...
DAYS = [
 dict(key="d2", no="10月1日", place="店舗・事務所", theme="開店前と閉店後に撮る。購入シーンだけ営業中",
      date="10月1日（木）　※仮", date2="2026年10月1日（木）　※仮",
      toc=[("05", "永尾社長・くさがやさんのインタビュー、店内、企画・設計"),
           ("07", "五シーンすべて（事務所の机・正面固定）")],
      cond="<b>店舗で撮るものは開店前と閉店後。購入シーンだけ営業中に撮る。</b>"
           "開店を十一時、閉店を十九時として組んでいるので、実際の時刻に合わせて前後にずらす。"
           "昼から夕方は空くので、その間は編集にあてる。",
      gear="カメラ／三脚／ピンマイク 二本／ガンマイク／PLフィルター／白い板／"
           "鞄に入れる物 一式（A4書類・13インチPC・手帳・鍵・サングラス・ペン・傘・水筒）",
      keys=["インタビューは窓を横に置く。二人とも同じ椅子・同じ画角で撮る。",
            "<b>店内の雰囲気を撮るカットは、人を画に入れない。</b>接客のカットは人を入れる。",
            "⑦は三脚を立てたら最後まで触らない。十四カット全部が同じ画角。"],
      talks=[("9:25〜9:55", "永尾社長", "【開店前】店内（窓を横に・インタビューの一角）",
              "", "",
              [("05", 7)], "⑤ インタビュー"),
             ("10:00〜10:30", "くさがやさん", "【開店前】同じ場所（椅子も画角もそのまま）",
              "", "", [], "⑤ インタビュー（店）")],
      setups=[("9:00〜9:20", "【開店前】店内（ショーウィンドウ越し・棚）", [("05", 5)],
               "<b>人を画に入れない。</b>お客さまが入る前に撮る。"),
              ("10:30〜11:10", "【開店前】店舗の机（打ち合わせの場所）", [("05", 2), ("05", 3)],
               "<b>⑤の企画と設計。</b>机を一度組んだら、二シーンを続けて撮る。"),
              ("11:10〜19:00", "空き（編集にあてる）", [],
               "営業中は撮らない。⑦は閉店後に撮る。"),
              ("19:00〜20:15", "【閉店後】事務所（バックヤード）の机・正面固定",
               [("07", 1), ("07", 2), ("07", 3), ("07", 4), ("07", 5)],
               "十四カット全部が同じ画角。物を並べてから、一度通してもらって本番。"),
]),
 dict(key="d5", no="購入者の日", place="店舗", theme="購入者に出ていただく日。⑧と⑤の購入シーン",
      date="未定　※購入者が決まってから", date2="未定　※購入者が決まってから",
      toc=[("08", "店の前と店内、窓辺のインタビュー、外に出て使う場面、見送り"),
           ("05", "購入・使う（会計・手渡し・使っている手元）")],
      cond="<b>出演していただく購入者が決まってから日を決める。</b>ほかの日より後になってよい。"
           "⑧は開店前にまとめて撮り、<b>⑤の購入シーンだけ営業中に撮る。</b>"
           "開店を十一時として組んでいるので、実際の開店時刻に合わせて前後にずらす。",
      gear="カメラ／三脚／ピンマイク／ガンマイク／PLフィルター／白い板／手前に置く植物／商品／ノートPC",
      keys=["<b>購入者の拘束は 9:00〜11:45。</b>⑧と⑤の購入シーンに続けて出ていただく。",
            "<b>つかみと見送りは同じ立ち位置でまとめて撮る。</b>外を先に済ませ、そのあと中へ入る。",
            "<b>窓辺のインタビューは最後。</b>撮り慣れて、落ち着いてから話してもらう。",
            "顔出しの範囲を、撮る前に本人へ確認する。"],
      setups=[("9:00〜9:35", "【開店前】店の前（外から中へ・見送り）", [("08", 1), ("08", 7)],
               "<b>つかみと見送りは同じ立ち位置。設営は一度でよい。</b>外から中へ入るところは切らずに追う。"),
              ("9:35〜9:55", "【開店前】店の前の通り", [("08", 5)],
               "外に出て使っている場面。ここで外は終わり。"),
              ("9:55〜10:10", "【開店前】店内（ロゴの壁）", [("08", 2)],
               "<b>人を画に入れない。</b>棚の乱れを直してから回す。"),
              ("10:10〜11:00", "【開店前】店の窓辺（手前に植物を置いてボカす）", [("08", 3), ("08", 4), ("08", 6)],
               "<b>インタビューの三シーンを続けて撮る。</b>カメラは一度も動かさない。"),
              ("11:15〜11:45", "【営業中】店内のレジまわり", [("05", 6)],
               "<b>購入シーンだけ営業中に撮る。</b>同じ購入者に、会計から使う場面まで出ていただく。"),
]),
 dict(key="d4", no="10月2日", place="東京国際フォーラム → 表参道",
      ...
      cond="出演者と衣装・鞄 四色ぶんが揃ってから。会場の撮影申請も先に出す。<b>曇りの日に合わせる。予備日は10月6日（火）の日中に仮で置く</b>（同じ日の終業後に③を撮るので、日中は空いている）。<b>表参道はどの時間も人が多い。</b>人を避けるなら、④⑤だけ別の日の 8:00〜9:30 に回す手もある。",
      ...
      setups=[("9:00〜10:10", "ガラス棟ロビー（北端 → 柱の列 → ガラスの扉 → 南端の階段前）",
              ...
              ("11:05〜13:10", "移動と昼休み（有楽町 → 表参道）", [],
               "<b>出演者の拘束は 9:00〜15:25。</b>移動と休憩を含めて先に伝えておく。"),
...
CAST = [
 ...
 ("⑧", "08", "購入者の日", "映る（バストショット）",
  ...
  "顔出しの範囲を、撮る前に本人へ確認する。<br>名前は出さず「購入者Aさん」とだけ出す。<br><b>拘束は 9:00〜11:45。⑧と⑤の購入シーンに続けて出ていただく。日は購入者が決まってから決める。</b>"),
...
# 実際に描かせて測った高さ
HEIGHT = {"cover": 1138, "d2": 1847, "d5": 1702, "d4": 2480, "d3": 3071, "d1": 1325, "cast": 1217}
