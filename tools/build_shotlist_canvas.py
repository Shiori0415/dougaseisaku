...
DAYS = [
 dict(key="d1", no="一日目", place="向島工房", theme="三脚を据えて撮る日",
      lead="三脚を据えて撮る二本を、同じ建物の中で続けて撮る。機材の出し入れが一度で済む。",
      ...
      blocks=[("07", [1, 2, 3, 4, 5]), ("02", [1, 2, 3, 4, 5, 6, 7, 8, 9])]),
 ...
 dict(key="d3", no="三日目", place="向島工房", theme="会社を語る日／暗くして撮る日",
      lead="午前に空撮と永尾社長のインタビューを済ませ、午後に窓を塞いで暗い一本を撮る。",
 ...
 dict(key="d4", no="四日目", place="店舗・屋上", theme="店とお客さまの日",
      lead="店で撮る二本をまとめる。午前が店の顔、午後が購入者のインタビュー。",
      gear="カメラ／三脚／ピンマイク／PLフィルター／白い板／手前に置く植物",
      keys=["店の外観は午前。看板に日が当たる時間に撮る。",
            "ショーウィンドウ越しは、向かいの建物が映り込まない立ち位置を先に探す。",
            "購入者のインタビューはバストショットで固定。話が長くなっても画角は変えない。"],
      blocks=[("05", [2, 6, 7]), ("08", [1, 2, 3, 4, 5, 6, 7, 8])]),

 dict(key="d5", no="五日目", place="すみだリバーウォーク・隅田公園・東京ミズマチ", theme="曇りの日に撮る",
 ...
 dict(key="d6", no="六日目", place="東京国際フォーラム ガラス棟", theme="四色をそろえる日",
...
<div style="width: {PAGE_W}px; height: {HEIGHT["cover"]}px; background: #ffffff; padding: {MARGIN}px; box-sizing: border-box; display: flex; flex-direction: column; gap: 22px">
  ...
  <div style="font-size: 18px; line-height: 1.9; color: {MUTED}">
    同じ場所で撮る本をまとめ、機材の出し入れと移動が最も少なくなる順に並べています。<br>
    前半の三日は向島工房だけで終わり、四日目に店舗、五日目に街、六日目に東京国際フォーラムへ移ります。<br>
  ...
  <div style="border-top: 1px solid {LINE}; padding-top: 20px; display: flex; gap: 44px">
    <div style="flex: 1">
      ...
      <div style="font-size: 17px; line-height: 1.9; color: {INK}">
        四日目 ── 購入者インタビューの出演者と、顔出しの範囲。<br>
        六日目 ── 出演者と、衣装・鞄 四色ぶん。会場の撮影申請。<br>
        五日目 ── 曇りの日に合わせるため、前後に予備日を一日置く。
      </div>
    </div>
    <div style="flex: 1">
...
# 実際に描かせて測った高さ
HEIGHT = {"cover": 1075, "d1": 1692, "d2": 1324, "d3": 1416,
          "d4": 1465, "d5": 850, "d6": 984}
