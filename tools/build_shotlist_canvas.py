...
MOVED = {("05", 7): ([1, 2], "1日目"),
         ("02", 4): ([2, 3, 4], "2日目"),
         ("06", 2): ([3], "4日目"), ("06", 8): ([3], "4日目"),
         ("03", 3): ([4], "4日目"), ("03", 5): ([1], "4日目")}
REUSE = {("05", 4): "⑥から引っ張る"}
...
("01",3):("モデル2", "三色目の衣装・鞄"),
("01",4):("モデル2", "四色目の衣装・鞄"),
("01",5):("モデル2、通行人", "四色ぶんの衣装・鞄"),
("01",6):("モデル2", "鞄 四種類、ロゴ"),
("02",1):("佐藤さん", "革のロールが並ぶ棚"),
("02",2):("佐藤さん", "抜き型、革、裁断機"),
("02",3):("佐藤さん", "革 二枚、型"),
("02",4):("佐藤さん", "ミシン、糸、革、手縫いの道具"),
("02",5):("佐藤さん", "刃、道具、革"),
("02",6):("佐藤さん", "コバ塗料、道具、断面"),
("02",7):("佐藤さん", "木槌、道具、立ち上がった鞄"),
("02",8):("佐藤さん", "完成した鞄、ロゴ"),
...
("04",1):("モデル3", "鞄、鉄のフェンス"),
("04",2):("モデル3", "鞄、中に入れる持ち物"),
("04",3):("モデル3", "鞄"),
("04",4):("モデル3", "鞄、持ち物、靴、ロゴ"),
("05",1):("―", "表参道の並木通り（高いところから）"),
("05",2):("永尾社長、くさがやさん（または内藤さん・渡辺さん）", "色見本、革、店舗の机"),
("05",3):("永尾社長、くさがやさん（または内藤さん・渡辺さん）", "図面、型紙"),
("05",4):("―", "―"),
("05",5):("―", "店内の商品、ショーウィンドウ"),
...
("06",9):("―", "文字のカード、ロゴ"),
("07",1):("モデル1", "鞄、机"),
("07",2):("モデル1", "A4書類、13インチPC"),
("07",3):("モデル1", "手帳、鍵、サングラス、ペン、傘、水筒"),
("07",4):("モデル1", "鞄"),
("07",5):("―", "何も乗っていない机、ロゴ"),
...
DAYS = [
 dict(key="d1", no="1日目", place="店舗・事務所", theme="⑤の語りと机まわり、そのまま⑦",
      date="1日目", date2="1日目　── 10月中（日にちは未定・月曜か火曜）",
 ...
 dict(key="d2", no="2日目", place="店舗", theme="購入者と、くさがやさんの手仕事",
      date="2日目", date2="2日目　── 10月中（日にちは未定・月曜か火曜）",
      ...
      keys=["購入者の拘束は 9:00〜12:35。",
            ...
            "②③は、光の向き・高さ・距離とケルビンを数字で控える。4日目に同じに組む。"],
      setups=[('9:00〜9:55', '店の前（外から中へ・見送り・開店準備）', [("08", 1), ("08", 7)],
              ...
              ('13:40〜14:05', '店舗の作業台（②・くさがやさんの引きと目元）', [],
               '<b>手元の品が読める寄りは撮らない。</b>4日目と同じ光の向き・高さ・距離、'
               '同じケルビンで組む。',
              ...
              ('14:05〜16:07', '店舗内の作業台（③・黒スチレンボード）',
               ...
               '佐藤さんの二枚（S3 4枚目の目元・S5 1枚目の引き）だけ4日目に工房で撮る。')]),

 dict(key="d3", no="3日目", place="向島工房", theme="⑥を工程順に、平日の工房で",
      date="3日目", date2="3日目　── 10月中（日にちは未定・平日）",
      toc=[("06", "工房の全体と機械、革と金型、裁断、下仕事、縫製、検品、梱包"
                  "　── <b>日が分かれることがある</b>")],
      cond="<b>平日に撮る。</b>機械が動いていて、人がいる時間。"
           "<b>画に入れられるのは、BROOKLYN MUSEUMの品を作っているところだけ。</b>"
           "他社の品が流れている工程はその日に撮れないので、"
           "<b>⑥は一日で撮り切れず、日が分かれることがある。</b>"
           "工房に当日どの品を流すかを先に聞き、撮れる工程から順に回す。"
           "残った工程は別の平日に撮り足す。",
      gear="カメラ（広角）／三脚／ガンマイク／白い板（発泡スチロール）／脚立／養生テープ",
      keys=["画に入れるのはブルックリンの品だけ。<b>他社の品が流れていたら、その工程は撮らずに次へ進む。</b>",
            "<b>撮れた工程に印をつけておく。</b>抜けた工程だけを、別の平日に撮り足す。",
      ...
      setups=[("9:30〜9:50", "工房の全体が入る位置", [("06", 1)],
              ...
              ("10:10〜10:45", "革の棚・金型の棚", [("06", 3)], "棚の奥行きが出る位置から。"
               '<div style="margin-top:9px; padding:9px 11px; border:1px solid #a8672a; '
               'border-radius:5px; background:#fdf6ee; color:#a8672a">'
               '<b>革を作っているカワゼンさん・ミヤツグさんにも撮影をお願いしたい。'
               '別の日に撮る。日程は未定。</b></div>'),
 ...
 dict(key="d4", no="4日目", place="向島工房（休日）", theme="②を工程順に、静かな工房で",
      date="4日目", date2="4日目　── 10月中（日にちは未定・休日）",
      toc=[("02", "八シーンすべて　佐藤さんが一つの鞄を頭から作りきる"),
           ("06", "佐藤さんのインタビュー"),
           ("03", "佐藤さんの二枚（目元と引き・工房の一角）")],
      cond="<b>工房の休日に撮る。</b>工房が止まっているので、出入りの音が入らない。"
      ...
      keys=["<b>②は一つの鞄を頭から作りきる。</b>作る順のとおりで、順番は入れ替えられない。",
            "<b>休日なので、工房にいるのは佐藤さんだけ。</b>引きにも佐藤さんしか入らない。",
            "<b>③でこの日に撮るのは佐藤さんの二枚だけ。工程シーンは2日目に全部ある。</b>",
            "③は2日目に組んだのと同じ光・同じ距離・同じケルビンで組む。",
            "②のコバ塗り以外は、機械の音を主役にする。"],
      ...
      setups=[("9:30〜9:50", "革のロールが並ぶ棚", [("02", 1)], "一枚だけ、三秒。"),
              ...
              ("14:30〜14:55", "工房の一角（③・黒スチレンボード）", [],
               "<b>2日目と同じ光の向き・高さ・距離・同じケルビンで組む。</b>"
 ...
 dict(key="d5", no="5日目", place="東京国際フォーラム → 表参道",
      theme="出演者を立てて撮る二本", date="5日目", date2="5日目　── 10月中（日にちは未定）",
      toc=[("01", "六シーンすべて（外の歩道 → 横断歩道 → 渡った先の通り → 扉 → ロビー）"),
           ("04", "柵（最初と最後）、置き画、けやき並木の歩道を歩く"),
           ("05", "冒頭の俯瞰（表参道を高いところから）")],
      cond="<b>モデル2（①）とモデル3（④）の二人。</b>衣装・鞄 四色ぶんが揃ってから。永尾さんの車も要る（①の合図）。"
      ...
      setups=[('9:00〜9:54', '丸の内仲通りのけやき並木 → 東京国際フォーラム東交差点の横断歩道',
              ...
              ('12:41〜13:35', '昼休み', [], 'モデル2の拘束は 9:00〜11:45。モデル3は 13:30〜16:05。'),
              ('13:35〜14:25', 'けやき並木の歩道（柵・手すり・青山通り寄りの端）', [("04", 1), ("04", 4)],
               '最初と最後は同じ柵。設営は一度。'),
              ('14:35〜15:03', 'ベンチか石段（真俯瞰が組めるところ）', [("04", 2)],
               '<b>モデル3の手が真俯瞰で映る。</b>鞄に入れる持ち物を先に並べておく。'),
              ('15:10〜15:35', 'けやき並木の裏の路地（人がほとんど通らない側）', [("04", 3)],
               '人の少ない裏側で。'),
              ('15:35〜16:05', 'けやき並木の歩道（モデル3はここまで）', [],
               '切らずに長めに三本。',
               '<b>⑤ 使用シーンの追加素材</b><br>街で鞄を持って歩く・手に取る・肩に掛け直す。',
               'モデル3<div style="color: #5b6266; margin-top: 3px">鞄</div>'),
              ('16:15〜16:37', 'Bロール（まとめて撮る）', [("05", 1)],
               '人が映らないカット。<b>モデル3が帰ったあとでよい。</b>')]),
]
...
<div style="width: {PAGE_W}px; height: {HEIGHT["cover"]}px; background: #ffffff; padding: {MARGIN}px; box-sizing: border-box; display: flex; flex-direction: column; gap: 22px">
  <div style="border-bottom: 2px solid {INK}; padding-bottom: 16px">
    ...
    <div style="display: flex; align-items: baseline; gap: 18px; margin-top: 10px">
      ...
      <div style="margin-left: auto; font-size: 18px; font-weight: 700; color: {INK}">全八本　{tot_s}カット　／　撮影 5日</div>
...
CAST = [
 ...
 ("⑤⑥", None, "1日目・4日目",
  "襟元と髪だけ、撮る直前に鏡で見る。",
  "ふだんの仕事着。<br>永尾社長・くさがやさんは1日目、佐藤さんは4日目。",
