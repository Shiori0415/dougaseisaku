...
("05",4):("―（⑥の画を使う）", "―"),
("05",5):("―", "店内の商品、ショーウィンドウ"),
("05",6):("くさがやさん（または内藤さん・渡辺さん。⑧S1で開店準備をした人と同じ）、購入者",
 "商品、ノートPC"),
("05",7):("永尾社長", "ロゴ"),
...
("07",4):("出演者（手と肩だけ）", "鞄"),
("07",5):("―", "何も乗っていない机、ロゴ"),
("08",1):("購入者（顔まで入る）、くさがやさん（または内藤さん・渡辺さん）",
 "店の外観、入口の扉、開店準備の商品と棚、レジ"),
("08",2):("―", "商品、ロゴの壁"),
("08",3):("購入者（顔まで入る）", "商品、手前に置く植物"),
("08",4):("購入者（顔まで入る）", "商品、窓辺"),
("08",5):("購入者（顔まで入る）", "鞄、店の前の通り、空"),
("08",6):("購入者（顔まで入る）", "表情のアップ"),
("08",7):("購入者、くさがやさん（見送る役）　ともに顔まで入る", "ロゴ"),
("08",8):("―", "ロゴ"),
}
...
DAYS = [
 dict(key="d1", no="一日目", place="店舗・事務所", theme="⑤の語りと机まわり、そのまま⑦",
      ...
      setups=[('10:15〜11:00', '店舗の机（打ち合わせの場所）', [("05", 2), ("05", 3)],
              ...
              ('12:10〜13:10', '昼休み', [], ''),
              ('13:10〜14:10', 'Bロール（まとめて撮る）', [("05", 5)],
               '人が映らないカット。<b>一時間取る。</b>棚・照明・ショーウィンドウを、'
               'インタビューで出た言葉に合わせて撮り足せるようにしておく。')]),
 ...
 dict(key="d4", no="四日目", place="向島工房（休日）", theme="②を工程順に、静かな工房で",
      ...
      keys=["<b>②は一つの鞄を頭から作りきる。</b>作る順のとおりで、順番は入れ替えられない。",
            "<b>手元が映る寄りはすべて佐藤さん。</b>くさがやさんの引きと目元だけ二日目に撮ってある。",
            "③は二日目に組んだのと同じ光・同じ距離・同じケルビンで組む。",
            "②③とも、セットの頭でグレーカードを一枚撮る。",
            "②のコバ塗り以外は、機械の音を主役にする。"],
      talks=[("15:00〜15:30", "佐藤さん", "向島工房（窓を横に・他社の品が入らない一角）",
              "", "", [], "⑥ インタビュー")],
      setups=[("9:30〜9:50", "革のロールが並ぶ棚", [("02", 1)], "一枚だけ、三秒。"),
              ...
              ("14:30〜14:50", "工房の一角（③・黒スチレンボード）", [("03", 4)],
 ...
 dict(key="d5", no="五日目", place="東京国際フォーラム → 表参道",
      ...
      keys=["<b>①は東京国際フォーラムのまわりで撮り切る。</b>外の歩道 → 横断歩道 → 渡った先の通り → 扉 → ロビー。",
            ...
            "④は三脚を止めて、背景だけを動かす。",
            "雨上がりの翌日は避ける。"],
      setups=[('9:00〜9:54', '東京国際フォーラム 地上広場のけやきぎわ → 前の横断歩道',
               [("01", 1), ("01", 2)],
               '一色目はけやきとガラス棟を背景に。二色目は<b>横断歩道を渡りきるところ</b>で、'
               '目の前を車が通り過ぎた一コマでつなぐ。'),
              ('9:54〜10:31', '渡った先の通り（丸の内仲通り側のけやき並木）→ ガラス棟の扉',
               [("01", 3), ("01", 4)],
               '三色目は<b>けやきの後ろを通った一瞬</b>、四色目は<b>扉をまたいだ一コマ</b>でつなぐ。'
               'S3とS4のあいだで来た道を戻るが、画角が違うので見ている人には分からない。'
               '<b>扉の画に横断歩道を入れない。</b>'),
