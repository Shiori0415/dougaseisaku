...
DAYS = [
 dict(key="d2", no="一日目", place="店舗・事務所", theme="開店前に⑤、そのまま⑦",
      date="一日目", date2="一日目　── 10月中（日にちは未定）",
      toc=[("05", "永尾社長・くさがやさんのインタビュー、店内、企画・設計"),
           ("07", "五シーンすべて（事務所の机・正面固定）")],
      cond="⑤は開店前（開店 十一時）。⑦はバックヤードかレンタルスペースなので営業時間は関係ない。",
      gear="カメラ／三脚／ピンマイク 二本／ガンマイク／PLフィルター／白い板／"
           "鞄に入れる物 一式（A4書類・13インチPC・手帳・鍵・サングラス・ペン・傘・水筒）",
      keys=["インタビューは窓を横に。二人は別の場所・別の画角。",
            "店内の雰囲気は人を入れない。接客は人を入れる。",
            "⑦は三脚を立てたら最後まで触らない。"],
      talks=[("9:25〜9:55", "永尾社長", "【開店前】店内（窓を横に・インタビューの一角）",
              "", "",
              [("05", 7)], "⑤ インタビュー"),
             ("10:00〜10:30", "くさがやさん", "【開店前】店内の別の一角（棚を背に）",
              "", "永尾社長とは別の場所・別の画角。横顔で撮る。", [], "⑤ インタビュー（店）")],
      setups=[('10:30〜10:50', '【開店前】店舗の机（打ち合わせの場所）', [("05", 2)],
               '机は一度組んで二シーン続けて。'),
              ('11:20〜11:50', 'バックヤード、またはレンタルスペース', [("07", 1), ("07", 4)],
               '設計が終わり次第。十四カット全部が同じ画角。'),
              ('12:00〜13:25', 'Bロール（まとめて撮る）', [("05", 5), ("05", 3), ("07", 2), ("07", 3), ("07", 5)],
               '人が映らないカット。まとめて撮る。')]),
 dict(key="d5", no="二日目", place="店舗", theme="午前は購入者、午後は③",
      date="二日目", date2="二日目　── 10月中（日にちは未定）",
      toc=[("08", "店の前と店内、窓辺のインタビュー、外に出て使う場面、見送り"),
           ("05", "購入・使う（会計・手渡し・使っている手元）"),
           ("03", "五シーンすべて（店舗内の作業台）")],
      cond="⑧は開店前、⑤の購入シーンだけ営業中。開店 十一時。"
           "③は午後、店舗内の作業台。窓の光が入らない一角を選ぶ。向島工房でやる場合は終業後か休日。",
      gear="カメラ／三脚／ピンマイク／ガンマイク／PLフィルター／白い板／手前に置く植物／商品／ノートPC／黒スチレンボード／ライト一灯",
      keys=["購入者の拘束は 9:00〜11:45。",
            "外を先に済ませ、そのあと中へ。",
            "窓辺のインタビューは最後。",
            "③はライト一灯だけ。当たっていない側は黒く落とす。"],
      setups=[('9:00〜9:35', '【開店前】店の前（外から中へ・見送り）', [("08", 1), ("08", 7)],
               ''),
              ('9:35〜9:55', '【開店前】店の前の通り', [("08", 5)],
               ''),
              ('10:10〜11:00', '【開店前】店の窓辺（手前に植物を置いてボカす）', [("08", 3), ("08", 4), ("08", 6)],
               '三シーン続けて。カメラは動かさない。'),
              ('11:15〜11:45', '【営業中】店内のレジまわり', [("05", 6)],
               '同じ購入者に、会計から使う場面まで。'),
              ('11:55〜12:10', 'Bロール（まとめて撮る）', [("08", 2)],
               '人が映らないカット。まとめて撮る。'),
              ('12:10〜13:00', '昼休み', [], ''),
              ('13:00〜14:40', '店舗内の作業台（③）', [("03", 1), ("03", 2), ("03", 3), ("03", 4), ("03", 5)],
               '台を一度組んだら五シーン続けて。')]),
 dict(key="d3", no="三日目", place="向島工房", theme="佐藤さんのインタビューと、機械の前ごとに二本",
      date="三日目", date2="三日目　── 10月中（日にちは未定）",
      toc=[("06", "佐藤さんのインタビュー、工房の全体と機械、革と金型、裁断、下仕事、縫製、検品、梱包"),
           ("02", "八シーンすべて（革の棚・裁断機・作業台・ミシン・下仕事・コバ塗り）"),
],
      cond="<b>佐藤さんの休日に合わせる。</b>工房は止まっているので、機械は佐藤さんに動かしてもらう。他の人が映る画は撮れない。",
      gear="カメラ（広角）／三脚／ガンマイク／ピンマイク／白い板（発泡スチロール）／脚立／養生テープ",
      keys=["画に入れるのはブルックリンの品だけ。",
            "蛍光灯と窓の光を混ぜない。明るさとWBはマニュアル固定。",
            "②のコバ塗りは切らずに六秒回す。",
            "②は作る順のとおり。順番は入れ替えられない。",
            "機械の前を離れる前に、②と⑥が両方撮れているか確かめる。",
 ...
 dict(key="d4", no="四日目", place="東京国際フォーラム → 表参道",
      theme="出演者を立てて撮る二本", date="四日目", date2="四日目　── 10月中（日にちは未定）",
      toc=[("01", "ロビーの四シーン（一色目〜四色目）"),
           ("04", "柵（最初と最後）、置き画、けやき並木の歩道を歩く"),
           ("05", "冒頭の俯瞰（表参道を高いところから）")],
      cond="出演者と衣装・鞄 四色ぶんが揃ってから。永尾さんの車も要る（②の合図）。曇りの日に合わせる。予備日を一日とる。",
...
CAST = [
 ...
 ("⑤⑥", None, "一日目・三日目", "映る（バストショット・横顔）",
  "襟元と髪だけ、撮る直前に鏡で見る。",
  "ふだんの仕事着。<br>永尾社長・くさがやさんは一日目、佐藤さんは三日目。",
...
# 実際に描かせて測った高さ
HEIGHT = {"cover": 819, "d2": 1461, "d5": 1850, "d3": 2350, "d4": 1786, "cast": 1217}
