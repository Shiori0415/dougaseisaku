...
DAYS = [
 dict(key="d2", no="10月1日", place="店舗・事務所", theme="開店前に⑤、そのまま⑦",
      ...
      setups=[('10:30〜10:50', '【開店前】店舗の机（打ち合わせの場所）', [("05", 2)],
              ...
              ('12:00〜13:25', 'Bロール（まとめて撮る）', [("05", 5), ("05", 3), ("07", 2), ("07", 3), ("07", 5)],
               '人が映らないカット。まとめて撮る。')]),
 dict(key="d5", no="10月2日", place="店舗", theme="午前。購入者に出ていただく",
      date="10月2日（金）　※仮", date2="2026年10月2日（金）　※仮",
      toc=[("08", "店の前と店内、窓辺のインタビュー、外に出て使う場面、見送り"),
           ("05", "購入・使う（会計・手渡し・使っている手元）")],
      cond="購入者が決まってから日を決める。⑧は開店前、⑤の購入シーンだけ営業中。開店 十一時。",
      gear="カメラ／三脚／ピンマイク／ガンマイク／PLフィルター／白い板／手前に置く植物／商品／ノートPC",
      ...
      setups=[('9:00〜9:35', '【開店前】店の前（外から中へ・見送り）', [("08", 1), ("08", 7)],
              ...
              ('11:55〜12:10', 'Bロール（まとめて撮る）', [("08", 2)],
               '人が映らないカット。まとめて撮る。')]),
 dict(key="d1", no="10月2日 午後", place="店舗内の作業台", theme="午後。一灯で撮る",
      date="10月2日（金）午後　※仮", date2="2026年10月2日（金）午後　※仮",
      toc=[("03", "五シーンすべて（店舗内の作業台）")],
      cond="店舗内の作業台で撮る。向島工房でやる場合は、別の日の終業後か休日。",
      gear="カメラ／三脚／白い板／黒スチレンボード／ライト一灯",
      keys=["窓の光が入らない一角を選ぶ。工房でやる場合は終業後か休日。",
            "ライト一灯だけ。当たっていない側は黒く落とす。",
            "三脚は必須。ピントは手で合わせて固定。",
            "画に入れるのはブルックリンの品だけ。"],
      setups=[('13:00〜14:40', 'Bロール（まとめて撮る）', [("03", 1), ("03", 2), ("03", 3), ("03", 4), ("03", 5)],
               '人が映らないカット。まとめて撮る。')]),
 ...
 dict(key="d4", no="10月6日", place="表参道",
      theme="出演者を立てて撮る二本", date="10月6日（火）　※仮", date2="2026年10月6日（火）　※仮",
      toc=[("01", "ロビーの四シーン（一色目〜四色目）"),
           ("04", "柵（最初と最後）、置き画、けやき並木の歩道を歩く"),
           ("05", "冒頭の俯瞰（表参道を高いところから）")],
      cond="出演者と衣装・鞄 四色ぶんが揃ってから。永尾さんの車も要る（②の合図）。曇りの日に合わせる。予備日は10月7日（水）。",
      ...
      keys=["四色ぶんは同じ立ち位置。床にテープを貼る。",
            ...
            "④は三脚を止めて、背景だけを動かす。",
            "雨上がりの翌日は避ける。"],
      setups=[('9:00〜10:10', 'けやき並木の歩道（並木通り → 車道ぎわ → 木の前 → ドアの前）', [("01", 1), ("01", 2), ("01", 3), ("01", 4)],
               '進む側の四シーン。切り替わりは 車・木・ドア。'),
              ('10:20〜11:20', '室内の廊下（レンタルスペース）', [("01", 5), ("01", 6)],
               '廊下を歩く。四色を並べるポーズもここで。'),
              ('11:30〜13:10', '昼休み', [],
               '出演者の拘束は 9:00〜15:25。'),
...
# 実際に描かせて測った高さ
HEIGHT = {"cover": 860, "d2": 1461, "d5": 1266, "d1": 1035, "d3": 2350, "d4": 1731, "cast": 1217}
