...
("01",6):("モデル 四色ぶん", "鞄 四種類、ロゴ"),
("02",1):("―", "革のロールが並ぶ棚"),
("02",2):("佐藤さん（手だけ）", "抜き型、革、裁断機"),
("02",3):("佐藤さん（手だけ）", "革 二枚、型"),
("02",4):("佐藤さん（手と上半身）", "ミシン、糸、革"),
("02",5):("くさがやさん（手だけ）", "刃、道具、革"),
("02",6):("くさがやさん（手だけ）", "コバ塗料、道具、断面"),
("02",7):("佐藤さん（手と上半身）", "木槌、道具、立ち上がった鞄"),
("02",8):("佐藤さん（手だけ）", "完成した鞄、ロゴ"),
("03",1):("―", "鞄の一部（光の帯だけ）、黒スチレンボード"),
("03",2):("くさがやさん（手だけ・時計と指輪は外す）", "革 二枚、染料、道具"),
("03",3):("くさがやさん（手と肩）", "革"),
("03",4):("くさがやさん（手だけ・白手袋）", "金具"),
("03",5):("佐藤さん（シルエット・1枚目だけ）", "完成品、工房の道具、ロゴ"),
("04",1):("モデル", "鞄、鉄のフェンス"),
("04",2):("手だけ", "鞄、中に入れる持ち物"),
("04",3):("モデル（後ろ姿）", "鞄"),
("04",4):("モデル", "鞄、持ち物、靴、ロゴ"),
...
DAYS = [
 dict(key="d1", no="一日目", place="店舗・事務所", theme="⑤の語りと机まわり、そのまま⑦",
      ...
      setups=[('10:15〜10:45', '店舗の机（打ち合わせの場所）', [("05", 2)],
               '机は一度組んで、企画のシーンを続けて。'),
              ('10:55〜11:25', '事務所の机（正面固定）', [("07", 1), ("07", 4)],
               '十四カット全部が同じ画角。'),
              ('11:25〜12:25', '昼休み', [], ''),
              ('12:25〜14:00', 'Bロール（まとめて撮る）',
               [("05", 5), ("05", 3), ("07", 2), ("07", 3), ("07", 5)],
               '人が映らないカット。まとめて撮る。')]),
 ...
 dict(key="d2", no="二日目", place="店舗", theme="購入者と、くさがやさんの手仕事",
      ...
      toc=[("08", "店の前と店内、窓辺のインタビュー、外に出て使う場面、見送り"),
           ...
           ("02", "下仕事とコバ塗り（店舗の作業台）　くさがやさんのシーン"),
           ("03", "五シーンすべて（店舗内の作業台）　くさがやさんのシーン")],
      cond="<b>店の休みの日（月曜か火曜）に撮る。</b>"
           "②の店舗ぶんは、<b>裁断と貼り合わせを先に済ませたパーツ</b>を用意しておく。"
           "③は窓の光が入らない一角で、蛍光灯を全部消す。",
      gear="カメラ／三脚／ガンマイク／レコーダー／PLフィルター／可変ND／白い板／手前に置く植物／"
           "商品／黒スチレンボード／ライト一灯／裁断と貼り合わせを済ませたパーツ",
      keys=["購入者の拘束は 9:00〜11:40。",
            "外を先に済ませ、そのあと中へ。",
            "③はライト一灯だけ。当たっていない側は黒く落とす。",
            "③の台を組むのは一度きり。組んだら五シーン続けて。"],
      setups=[('9:00〜9:35', '店の前（外から中へ・見送り）', [("08", 1), ("08", 7)], ''),
              ('9:35〜9:55', '店の前の通り', [("08", 5)], ''),
              ('10:10〜11:00', '店の窓辺（手前に植物を置いてボカす）',
               [("08", 3), ("08", 4), ("08", 6)], '三シーン続けて。カメラは動かさない。'),
              ('11:10〜11:40', '店内のレジまわり', [("05", 6)],
               '同じ購入者に、会計から使う場面まで。購入者はここまで。'),
              ('11:40〜12:30', '昼休み', [], ''),
              ('12:30〜12:45', 'Bロール（まとめて撮る）', [("08", 2)],
               '人が映らないカット。③の台に組み替える前に済ませる。'),
              ('12:45〜13:35', '店舗の作業台（②の下仕事とコバ塗り）', [("02", 5), ("02", 6)],
               'くさがやさんの手。コバ塗りは切らずに六秒回す。'),
              ('13:35〜15:25', '店舗内の作業台（③・黒スチレンボード）',
               [("03", 1), ("03", 2), ("03", 3), ("03", 4), ("03", 5)],
               '台を一度組んだら五シーン続けて。人が入る画はくさがやさんの肩から先だけ。')]),
 ...
 dict(key="d3", no="三日目", place="向島工房", theme="⑥を工程順に、平日の工房で",
      ...
      setups=[("9:30〜9:50", "工房の全体が入る位置", [("06", 1)],
              ...
              ("9:50〜10:10", "材料の棚と機械の並び", [("06", 2)],
               "佐藤さんの引きだけは四日目に撮る。"),
              ("10:10〜10:40", "革の棚・金型の棚", [("06", 3)], "棚の奥行きが出る位置から。"),
              ("10:40〜11:10", "裁断機のまわり", [("06", 4)], "⑤S4の手元もここから。"),
              ("11:10〜11:50", "下仕事の台", [("06", 5)], ""),
              ("11:50〜12:50", "昼休み", [], "工房の昼休みに合わせる。"),
              ("12:50〜13:30", "ミシンのまわり", [("06", 6)], "手縫いのカットも一つ撮る。"),
              ("13:30〜14:00", "検品の台", [("06", 7)], "ここで作業音がいったん消える。"),
              ("14:00〜14:30", "梱包の台", [("06", 8)], "紙とテープの音を拾う。")]),

 dict(key="d4", no="四日目", place="向島工房（休日）", theme="②を工程順に、静かな工房で",
      date="四日目", date2="四日目　── 10月中（日にちは未定・佐藤さんの休日）",
      toc=[("02", "革の棚・裁断・貼り合わせ・縫製・組み・完成　佐藤さんのシーン"),
           ("06", "佐藤さんのインタビュー"),
           ("03", "佐藤さんのシーン（工房の一角・黒スチレンボード）")],
      cond="<b>佐藤さんの休日に合わせる。</b>工房が止まっているので、出入りの音が入らない。"
           "②の録音にはこの日でないといけない。機械は佐藤さんに動かしてもらう。",
      gear="カメラ（広角）／マクロ／三脚／ガンマイク／レコーダー／白い板／脚立／"
           "黒スチレンボード／ライト一灯／②の店舗ぶんと同じ革・同じ品番",
      keys=["②は作る順のとおり。順番は入れ替えられない。",
            "下仕事とコバ塗りは二日目に撮ってある。ここでは撮らない。",
            "③の追加は、二日目に組んだのと同じ光・同じ距離で組む。",
            "②のコバ塗り以外は、機械の音を主役にする。"],
      talks=[("14:00〜14:30", "佐藤さん", "向島工房（窓を横に・他社の品が入らない一角）",
              "", "", [], "⑥ インタビュー")],
      setups=[("9:30〜9:50", "革のロールが並ぶ棚", [("02", 1)], "一枚だけ、三秒。"),
              ("9:50〜10:20", "裁断機のまわり", [("02", 2)], "型を置く・刃が入る・抜いたパーツ。"),
              ("10:20〜10:45", "作業台（真俯瞰の三脚が組める台）", [("02", 3)],
               "真俯瞰は台の上に直接置くか、脚立から。"),
              ("10:45〜11:25", "ミシンのまわり", [("02", 4)], "寄りと引きを交互に。"),
              ("11:25〜12:25", "昼休み", [], ""),
              ("12:25〜13:05", "作業台（引きが撮れる位置まで下がれるところ）",
               [("02", 7), ("02", 8)], "二シーンとも同じ画角。続けて撮る。"),
              ("13:15〜13:45", "工房の一角（③・黒スチレンボード）", [],
               "二日目と同じ光の向き・高さ・距離で組む。数字を控えてある。",
               "<b>③ 佐藤さんのシーン</b><br>手元の寄りと、肩から先だけが光に入る一枚。顔は闇に置く。",
 ...
 dict(key="d5", no="五日目", place="東京国際フォーラム → 表参道",
      ...
      keys=["四色ぶんは同じ立ち位置。床にテープを貼る。",
            "カメラの高さは胸で固定。",
            "表参道は人が多い。交差点から離れた端と裏の路地を使う。",
            "④は三脚を止めて、背景だけを動かす。",
            "雨上がりの翌日は避ける。"],
      setups=[('9:00〜10:00', '東京国際フォーラム ガラス棟ロビー（通路・ベンチ）', [("01", 5), ("01", 6)],
               '廊下を歩く。四色を並べるポーズもここで。'),
              ('10:00〜11:00', '移動（有楽町 → 表参道）', [], ''),
              ('11:00〜12:10', 'けやき並木の歩道（並木通り → 車道ぎわ → 木の前 → ドアの前）',
               [("01", 1), ("01", 2), ("01", 3), ("01", 4)],
               '進む側の四シーン。切り替わりは 車・木・ドア。'),
              ('12:10〜13:10', '昼休み', [], '出演者の拘束は 9:00〜15:25。'),
              ('13:10〜13:50', 'けやき並木の歩道（柵・手すり・青山通り寄りの端）', [("04", 1), ("04", 4)],
               '最初と最後は同じ柵。設営は一度。'),
              ('14:35〜14:55', 'けやき並木の裏の路地（人がほとんど通らない側）', [("04", 3)],
               '人の少ない裏側で。'),
              ('14:55〜15:25', 'けやき並木の歩道（出演者はここまで）', [],
               '切らずに長めに三本。',
               '<b>⑤ 使用シーンの追加素材</b><br>街で鞄を持って歩く・手に取る・肩に掛け直す。',
               'モデル<div style="color: #5b6266; margin-top: 3px">鞄</div>'),
              ('15:35〜16:20', 'Bロール（まとめて撮る）', [("04", 2), ("05", 1)],
               '人が映らないカット。まとめて撮る。')]),
]
...
CAST = [
 ...
 ("⑧", "08", "二日目", "映る（バストショット）",
  ...
  "顔出しの範囲を、撮る前に本人へ確認する。<br>名前は出さず「購入者Aさん」とだけ出す。<br>拘束は 9:00〜11:45。日は購入者が決まってから決める。"),
...
# 実際に描かせて測った高さ
HEIGHT = {"cover": 964, "d1": 1499, "d2": 2135, "d3": 1459, "d4": 1361, "d5": 1786, "cast": 1217}
