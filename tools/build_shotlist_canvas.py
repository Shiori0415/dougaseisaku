...
DAYS = [
 dict(key="d1", no="一日目", place="店舗・事務所", theme="⑤の語りと机まわり、そのまま⑦",
      ...
      setups=[('10:15〜10:45', '店舗の机（打ち合わせの場所）', [("05", 2)],
               '机は一度組んで、企画のシーンを続けて。'),
              ('10:55〜11:30', '事務所の机（正面固定）', [("07", 1), ("07", 4)],
               '十四カット全部が同じ画角。'),
              ('11:30〜12:30', '昼休み', [], ''),
              ('12:30〜14:15', 'Bロール（まとめて撮る）',
 ...
 dict(key="d2", no="二日目", place="店舗", theme="購入者と、くさがやさんの手仕事",
      ...
      keys=["購入者の拘束は 9:00〜12:20。",
            "外を先に済ませ、そのあと中へ。",
            "③はライト一灯だけ。当たっていない側は黒く落とす。",
            "③の台を組むのは一度きり。組んだら五シーン続けて。"],
      setups=[('9:00〜9:45', '店の前（外から中へ・見送り）', [("08", 1), ("08", 7)], ''),
              ('9:45〜10:10', '店の前の通り', [("08", 5)], ''),
              ('10:25〜11:35', '店の窓辺（手前に植物を置いてボカす）',
               [("08", 3), ("08", 4), ("08", 6)], '三シーン続けて。カメラは動かさない。'),
              ('11:45〜12:20', '店内のレジまわり', [("05", 6)],
               '同じ購入者に、会計から使う場面まで。購入者はここまで。'),
              ('12:20〜13:10', '昼休み', [], ''),
              ('13:10〜13:25', 'Bロール（まとめて撮る）', [("08", 2)],
               '人が映らないカット。③の台に組み替える前に済ませる。'),
              ('13:25〜14:20', '店舗の作業台（②の下仕事とコバ塗り）', [("02", 5), ("02", 6)],
               'くさがやさんの手。コバ塗りは切らずに六秒回す。'),
              ('14:20〜16:25', '店舗内の作業台（③・黒スチレンボード）',
 ...
 dict(key="d4", no="四日目", place="向島工房（休日）", theme="②を工程順に、静かな工房で",
      ...
      keys=["②は作る順のとおり。順番は入れ替えられない。",
            ...
            "②のコバ塗り以外は、機械の音を主役にする。"],
      talks=[("14:20〜14:50", "佐藤さん", "向島工房（窓を横に・他社の品が入らない一角）",
              "", "", [], "⑥ インタビュー")],
      setups=[("9:30〜9:50", "革のロールが並ぶ棚", [("02", 1)], "一枚だけ、三秒。"),
              ...
              ("11:35〜12:35", "昼休み", [], ""),
              ("12:35〜13:25", "作業台（引きが撮れる位置まで下がれるところ）",
               [("02", 7), ("02", 8)], "二シーンとも同じ画角。続けて撮る。"),
              ("13:35〜14:05", "工房の一角（③・黒スチレンボード）", [],
 ...
 dict(key="d5", no="五日目", place="東京国際フォーラム → 表参道",
      ...
      keys=["四色ぶんは同じ立ち位置。床にテープを貼る。",
            ...
            "④は三脚を止めて、背景だけを動かす。",
            "雨上がりの翌日は避ける。"],
      setups=[('9:00〜10:10', '東京国際フォーラム ガラス棟ロビー（通路・ベンチ）', [("01", 5), ("01", 6)],
               '廊下を歩く。四色を並べるポーズもここで。'),
              ('10:10〜11:10', '移動（有楽町 → 表参道）', [], ''),
              ('11:10〜12:35', 'けやき並木の歩道（並木通り → 車道ぎわ → 木の前 → ドアの前）',
               [("01", 1), ("01", 2), ("01", 3), ("01", 4)],
               '進む側の四シーン。切り替わりは 車・木・ドア。'),
              ('12:35〜13:35', '昼休み', [], '出演者の拘束は 9:00〜16:05。'),
              ...
              ('16:15〜17:05', 'Bロール（まとめて撮る）', [("04", 2), ("05", 1)],
               '人が映らないカット。まとめて撮る。')]),
]
